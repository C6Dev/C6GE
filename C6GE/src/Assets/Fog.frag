...
void main() {
    ...
    if (hasCubemap == 1) {
        envColor = texture(cubemap, normalize(LocalPos)).rgb;
        result = envColor; // Use cubemap as base color for cubemap objects
    } else {
        // Existing lighting calculation remains for non-cubemap objects
    }
